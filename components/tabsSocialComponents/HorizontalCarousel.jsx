...
function HorizontalCarousel({ mainPosts, setMainPosts }) {
    ...
    return (<>
        <div className={`w-full h-screen fixed  top-0 left-0 bg-white/60 backdrop-blur-sm flex justify-center items-end z-[45] md:items-center ${openPageId != null ? "block" : "hidden"}`}>
            <div
                id="openCarousel"
                ...
                <div className={` flex flex-col overflow-y-auto lg:gap-6 w-full max-w-[500px] h-full sm:w-[550px] max-h-[95vh] md:max-h-[85vh]  lg:max-w-full lg:w-full mt-0 md:-mt-1`} >
                    {mainPosts.map((post, index) => (
                        <div key={index} className={` pt-1 basis-1 relative h-full lg:`}>
                            <div className={`lg:grid grid-cols-2   bg-primary rounded-lg h-fit  `}>
                                <CarouselCardHeader
                                    post={post}
            ...
            {isMobile &&
                <div className={`${openCommentPage != undefined && isMobile ? "flex lg:hidden backdrop-blur-[2px]  " : "hidden"} z-[999] flex-col absolute justify-center  items-center  w-full  h-screen text-white top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2`}>
                    <div ref={commentRef} className="relative bg-black w-10/12 rounded-md max-w-[400px]">
                        ...
                        <div>
                            <CommentForm posts={mainPosts} index={openCommentPage} setMainPosts={setMainPosts} />
                        </div>
                    </div>
                </div>}
        </div>
