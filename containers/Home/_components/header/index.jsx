...
function Header() {
    ...
    const searching_keys = ["first_name", "last_name", "company_name", "job_title"]
    ...
    useEffect(() => {
        ...
        const timer = searchTerm !== "" ? setTimeout(() => {
            const new_dt = Search_Algorithm(Mock_data, searchTerm, 0.5, searching_keys, clickedKeys)
            setData(new_dt)
            setIsOpenSearchLoading(false)
        }, 1000) : null
                        ...
                        {isAuthenticated ?
                            <div className="w-2/4 h-auto flex items-center relative">
                                <Input onChange={e => setSearchTerm(e.target.value)} value={searchTerm}
                                       className="w-full text-foreground" placeholder={"Arama yap..."}/>
                                <AnimatePresence>
                                    {isOpenSearchResults
                                        ? <div
                                            className="h-72 overflow-y-auto w-full rounded-md border !absolute top-full mt-2 bg-foreground/70 flex flex-col">
                                            <div className="p-4 flex flex-col gap-4 h-72">
                                                <h4 className="text-base font-medium leading-none">Search Results</h4>
                                                {!isOpenSearchLoading
                                                    ? data.map((dt, idx) => (
                                                        <>
                                                            <div onClick={() => {
                                                                ...
                                                                setClickedKeys(prev => [...clickedKeys, {
                                                                    searchTerm: searchTerm,
                                                                    clicked: clickeds
                                                                }])
                                                            }} key={idx} className="text-sm flex items-center gap-4">
                                                                <img alt="" src={dt.avatar} loading="lazy"
